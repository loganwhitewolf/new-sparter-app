...
describe("transaction DAL query helpers", () => {
  ...
  it("uses explicit whitelisted sort branches", () => {
    expect(getTransactionSortColumn("occurredAt")).toBe(
      "transaction.occurredAt",
    );
    expect(getTransactionSortColumn("amount")).toBe(
      transactionAmountAbsSortKey,
    );
    expect(getTransactionSortColumn("description")).toBe(
      transactionDisplayTitleSortKey,
    );
    expect(getTransactionSortColumn("category")).toBe(
      transactionLinkedExpenseCategorySortKey,
    );
    expect(getTransactionSortColumn("platform")).toBe(
      transactionPlatformSortKey,
    );
  });
  ...
  it("builds orderBy array with id tiebreaker as LAST element (D-06)", () => {
    ...
    expect(
      buildTransactionOrderBy({ sort: "occurredAt", dir: "desc" }),
    ).toEqual([
      { op: "desc", column: "transaction.occurredAt" },
      { op: "desc", column: "transaction.id" },
    ]);
    expect(
      buildTransactionOrderBy({ sort: "description", dir: "asc" }),
    ).toEqual([
  ...
  it("keeps the list bounded and exposes display metadata for the /transactions table", () => {
    ...
    expect(transactionListSelect.expenseSubCategoryName).toMatchObject({
      op: "sql",
    });
  ...
  it("verifies the session, scopes transaction rows to the user and file owner, and applies filters safely", async () => {
    ...
    expect(mocks.orderByArgs[0]).toEqual({
      op: "asc",
      column: transactionAmountAbsSortKey,
    });
    expect(mocks.whereArgs[0]).toEqual({
      ...
      args: expect.arrayContaining([
        ...
        },
        {
          ...
          right: new Date("2026-01-01T00:00:00.000Z"),
        },
        {
          op: "lte",
          left: "transaction.occurredAt",
          right: new Date("2026-01-31T23:59:59.999Z"),
        },
  ...
  it("updateTransactionCustomTitle sets customTitle to the provided string", async () => {
    const { db } = await import("@/lib/db");
    await updateTransactionCustomTitle(
      db,
      "txn-id-2",
      "user-1",
      "My custom title",
  ...
  it("months filter adds OR(TO_CHAR(occurredAt, YYYY-MM) = ym) for each month", async () => {
    ...
    const monthsCondition = where.args.find((arg) => {
      ...
      | {
          op: string;
          args: { op: string; strings: string[]; values: unknown[] }[];
        }
      | undefined;
...
describe("getUncategorizedTransactionsByFileId", () => {
  ...
  it("returns only description+amount rows and filters by fileId + expense.subCategoryId IS NULL (POST-04)", async () => {
    const chain = makeWhereTerminalChain([
      { description: "AMAZON 123", amount: "-12.50" },
    ]);
    const { getUncategorizedTransactionsByFileId } =
      await import("@/lib/dal/transactions");
    const result = await getUncategorizedTransactionsByFileId(
      chain as never,
      "file-1",
      "user-1",
    );
  ...
  it("enforces ownership via innerJoin on importFile and expense (POST-03)", async () => {
    ...
    const { getUncategorizedTransactionsByFileId } =
      await import("@/lib/dal/transactions");
    await getUncategorizedTransactionsByFileId(
      chain as never,
      "file-1",
      "user-1",
    );
...
describe("getTopUncategorizedExpenses (R-OB-07 query)", () => {
  ...
  it("executes parameterized SQL with DISTINCT ON, sub_category_id IS NULL, total_amount < 0, ORDER BY ABS DESC, LIMIT (R-OB-07)", async () => {
    ...
    const sqlArg = mocks.executeArgs[0] as {
      strings: string[];
      values: unknown[];
    };
...
describe("transaction pairing select-shape contract (Phase 50 — PAIR-02)", () => {
  ...
  it("transactionListSelect includes pairedWithId as a sql fragment (PAIR-02)", () => {
    ...
    expect(
      (transactionListSelect as Record<string, unknown>).pairedWithId,
    ).toMatchObject({ op: "sql" });
  });
  ...
  it("transactionListSelect includes pairedDescription as a sql fragment (PAIR-02)", () => {
    ...
    expect(
      (transactionListSelect as Record<string, unknown>).pairedDescription,
    ).toMatchObject({ op: "sql" });
  });
  ...
  it("transactionListSelect includes pairedOccurredAt as a sql fragment (PAIR-02)", () => {
    ...
    expect(
      (transactionListSelect as Record<string, unknown>).pairedOccurredAt,
    ).toMatchObject({ op: "sql" });
  });
