...
const schema = vi.hoisted(() => {
  const table = (name: string, columns: string[]) =>
    Object.fromEntries([['_table', name], ...columns.map((column) => [column, `${name}.${column}`])])

  return {
    file: table('file', ['id', 'userId', 'originalName', 'displayName', 'objectKey', 'status']),
    transaction: table('transaction', ['id', 'userId', 'fileId', 'expenseId', 'amount', 'occurredAt']),
...
vi.mock('drizzle-orm', () => ({
  ...
  ne: (column: string, value: unknown): Condition => ({ op: 'ne', column, value }),
  inArray: (column: string, values: unknown[]): Condition => ({ op: 'inArray', column, values }),
...
function matchesCondition(row: Record<string, unknown>, condition?: Condition): boolean {
  if (!condition) return true

  if (condition.op === 'and') return condition.conditions.every((child) => matchesCondition(row, child))
...
class SelectQuery {
  ...
  from(table: { _table: string }) {
    ...
    this.isAggregateQuery = 'totalAmount' in this.shape && 'transactionCount' in this.shape
    return this
  }
  ...
  private execute() {
    if (this.tableName === 'file') {
      return this.state.files.filter((row) => matchesCondition(row as unknown as Record<string, unknown>, this.condition))
    }
    ...
    if (this.tableName === 'transaction') {
      ...
      for (const row of rows) {
        ...
        grouped.set(row.expenseId, [...(grouped.get(row.expenseId) ?? []), row])
      }

      return [...grouped.entries()].map(([expenseId, groupedRows]) => {
        const dates = groupedRows.map((row) => row.occurredAt)
        const total = groupedRows.reduce((sum, row) => sum + Number(row.amount), 0)
        return {
          expenseId,
          totalAmount: total.toFixed(2),
          transactionCount: groupedRows.length,
          firstTransactionAt: new Date(Math.min(...dates.map((date) => date.getTime()))),
          lastTransactionAt: new Date(Math.max(...dates.map((date) => date.getTime()))),
        }
      })
    }
    ...
    if (this.tableName === 'expenseClassificationHistory') {
      return this.state.histories
        .filter((row) => matchesCondition(row as unknown as Record<string, unknown>, this.condition))
...
class FakeDb {
  ...
  constructor(state: FakeState) {
    ...
    mocks.dbTransaction.mockImplementation(async (callback: (tx: FakeDb) => Promise<unknown>) => {
      ...
      try {
        ...
        this.destructiveWrites.push(...tx.destructiveWrites.map((write) => `rolled-back:${write}`))
        throw error
      }
    })
  }
    ...
    return {
      set: (values: Record<string, unknown>) => ({
        where: async (condition: Condition) => {
          ...
          if (table._table !== 'expense') throw new Error(`Unexpected update table ${table._table}`)

          const idCondition = condition.op === 'and'
            ? condition.conditions.find(
                (child): child is EqCondition => child.op === 'eq' && child.column === schema.expense.id,
              )
            : undefined
          if (this.failOnExpenseId && idCondition?.value === this.failOnExpenseId) {
            throw new Error('forced expense update failure')
          }

          for (const row of this.state.expenses) {
            if (matchesCondition(row as unknown as Record<string, unknown>, condition)) {
      ...
      where: async (condition: Condition) => {
        ...
        if (table._table === 'transaction') {
          this.state.transactions = this.state.transactions.filter(
            (row) => !matchesCondition(row as unknown as Record<string, unknown>, condition),
          )
          return
        }
...
function baseState(overrides: Partial<FakeState> = {}): FakeState {
  const state: FakeState = {
    ...
    histories: [
      ...
      { id: 2, userId: USER_ID, expenseId: 'expense-override', source: 'override' },
      { id: 3, userId: USER_ID, expenseId: 'expense-delete', source: 'system_pattern' },
    ],
  }
...
function makeExpense(id: string, totalAmount: string, transactionCount: number, importedFromFileId: string | null): ExpenseRow {
  return {
    id,
    userId: USER_ID,
    title: id,
...
describe('getImportDeletePreview', () => {
  it('returns sanitized impact counts and IDs without storage keys or raw rows', async () => {
    const { getImportDeletePreview } = await import('@/lib/services/import-deletion')

    const preview = await getImportDeletePreview({ userId: USER_ID, fileId: FILE_ID })

    expect(preview).toMatchObject({
      fileId: FILE_ID,
      displayName: 'January statement',
      transactionCount: 4,
      affectedExpenseIds: ['expense-delete', 'expense-manual', 'expense-override', 'expense-recalc'],
  ...
  it('rejects malformed file IDs before database writes', async () => {
    const { getImportDeletePreview, ImportDeleteError } = await import('@/lib/services/import-deletion')

    await expect(getImportDeletePreview({ userId: USER_ID, fileId: 'not-a-uuid' })).rejects.toBeInstanceOf(
      ImportDeleteError,
    )
  ...
  it('rejects non-owned imports as not found', async () => {
    ...
    await expect(getImportDeletePreview({ userId: OTHER_USER_ID, fileId: FILE_ID })).rejects.toMatchObject({
  ...
  it('rejects unsupported importing/analyzing statuses', async () => {
    fakeDb = new FakeDb(baseState({ files: [{ ...baseState().files[0]!, status: 'importing' }] }))
    const { getImportDeletePreview } = await import('@/lib/services/import-deletion')

    await expect(getImportDeletePreview({ userId: USER_ID, fileId: FILE_ID })).rejects.toMatchObject({
...
describe('deleteImport', () => {
  it('deletes linked transactions, reconciles affected expenses, preserves manual/override empties, and deletes the file last', async () => {
    ...
    expect(fakeDb.state.expenses.find((row) => row.id === 'expense-delete')).toBeUndefined()

    expect(fakeDb.state.expenses.find((row) => row.id === 'expense-recalc')).toMatchObject({
      totalAmount: '-10.00',
      transactionCount: 2,
    ...
    expect(fakeDb.state.expenses.find((row) => row.id === 'expense-manual')).toMatchObject({
      totalAmount: '0.00',
      transactionCount: 0,
      importedFromFileId: null,
    ...
    expect(fakeDb.state.expenses.find((row) => row.id === 'expense-override')).toMatchObject({
      totalAmount: '0.00',
      transactionCount: 0,
      importedFromFileId: null,
      firstTransactionAt: null,
  ...
  it('deletes an imported file with no linked transactions without touching expenses', async () => {
    fakeDb = new FakeDb(baseState({ transactions: [], expenses: [], histories: [] }))
    const { deleteImport } = await import('@/lib/services/import-deletion')
    ...
    expect(fakeDb.state.files).toEqual([])
    expect(fakeDb.destructiveWrites).toEqual(['delete:transaction', 'delete:file'])
  })

  it('rejects malformed file IDs before starting a transaction', async () => {
    const { deleteImport } = await import('@/lib/services/import-deletion')

    await expect(deleteImport({ userId: USER_ID, fileId: 'bad-id' })).rejects.toMatchObject({
  ...
  it('rolls back the whole transaction when reconciliation fails mid-delete', async () => {
    ...
    await expect(deleteImport({ userId: USER_ID, fileId: FILE_ID })).rejects.toMatchObject({
      code: 'delete_failed',
      message: 'Impossibile eliminare l’importazione.',
    })

    expect(fakeDb.state).toEqual(original)
    expect(fakeDb.destructiveWrites).toContain('rolled-back:delete:transaction')
    expect(fakeDb.state.files).toHaveLength(1)
    expect(fakeDb.state.transactions).toHaveLength(original.transactions.length)
