...
vi.mock("@/lib/validations/pattern", async () => {
  const actual = await vi.importActual<
    typeof import("../lib/validations/pattern")
  >("../lib/validations/pattern");
  return actual;
});

const { createPatternAction, updatePatternAction, deletePatternAction } =
  await import("../lib/actions/patterns");
...
function expectExactCategoryRevalidationRoutes() {
  ...
  expect(uniqueSortedPaths).toEqual(
    [...EXPECTED_CATEGORY_REVALIDATION_ROUTES].sort(),
  );
}
...
describe("pattern Server Actions", () => {
  ...
  describe("createPatternAction", () => {
    it("passes canonical schema output and session userId to the DAL", async () => {
      const result = await createPatternAction(
        { error: null },
        validCreateForm({ userId: "attacker-id" }),
      );
      ...
      expect(mocks.revalidatePath).toHaveBeenCalledWith("/settings/categories");
    });
    ...
    it("prevents mutation when auth lookup fails", async () => {
      ...
      await expect(
        createPatternAction({ error: null }, validCreateForm()),
    ...
    it("returns validation errors without writing when subcategory is missing", async () => {
      const result = await createPatternAction(
        { error: null },
        validCreateForm({ subCategoryId: null }),
      );
    ...
    it("returns validation errors without writing when confidence is outside range", async () => {
      const result = await createPatternAction(
        { error: null },
        validCreateForm({ confidence: "1.2" }),
      );
    ...
    it("returns unsupported flag validation errors without writing for /netflix/g", async () => {
      const result = await createPatternAction(
        { error: null },
        validCreateForm({ pattern: "/netflix/g" }),
      );
  ...
  describe("updatePatternAction", () => {
    it("passes canonical schema output, id, and session userId to the DAL", async () => {
      const result = await updatePatternAction(
        { error: null },
        validUpdateForm({ userId: "attacker-id" }),
      );
    ...
    it("prevents mutation when auth lookup fails", async () => {
      ...
      await expect(
        updatePatternAction({ error: null }, validUpdateForm()),
      ).rejects.toThrow("NEXT_REDIRECT");
      expect(mocks.updatePattern).not.toHaveBeenCalled();
      expect(mocks.revalidatePath).not.toHaveBeenCalled();
    });
    ...
    it("returns validation errors without writing for invalid confidence", async () => {
      const result = await updatePatternAction(
        { error: null },
        validUpdateForm({ confidence: "-0.1" }),
      );
    ...
    it("returns validation errors without writing for unsupported flags", async () => {
      const result = await updatePatternAction(
        { error: null },
        validUpdateForm({ pattern: "/netflix/g" }),
      );
    ...
    it("returns a generic error without leaking DAL failures", async () => {
      mocks.updatePattern.mockRejectedValueOnce(
        new Error("SQL timeout on categorization_pattern"),
      );
  ...
  describe("deletePatternAction", () => {
    it("deletes using the session userId and revalidates settings on success", async () => {
      const result = await deletePatternAction(
        { error: null },
        makeFormData({ id: "7", userId: "attacker-id" }),
      );
    ...
    it("prevents mutation when auth lookup fails", async () => {
      ...
      await expect(
        deletePatternAction({ error: null }, makeFormData({ id: "7" })),
      ).rejects.toThrow("NEXT_REDIRECT");
      expect(mocks.deletePattern).not.toHaveBeenCalled();
      expect(mocks.revalidatePath).not.toHaveBeenCalled();
    });
    ...
    it("returns a generic error without leaking DAL failures", async () => {
      mocks.deletePattern.mockRejectedValueOnce(
        new Error("database host is unreachable"),
      );
