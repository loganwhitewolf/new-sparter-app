...
// This page is lens-invariant (LSD-05): it renders no lens control at all, so it reads no
// `?lens=` searchParam either. No preset/year/tag searchParam is read (TAG-05's per-tag total
// is all-time and does not subscribe to the dashboard's global filters, LOCKED DECISION 1).
// getTagTotals failure surfaces the same dashed-box shape as the empty state, distinguished
// only by heading text (backstop — no dedicated fetch-failure test harness exists in this repo,
// per RESEARCH.md). JSX is constructed only after the try/catch resolves (never inside it) to
// satisfy the error-boundaries lint rule.
