...
**Standalone Expense** (Spesa a sé):
Una Expense staccata deliberatamente dall'aggregazione per `descriptionHash`, che contiene **una sola transazione** ed è esclusa sia dal raggruppamento per descrizione sia dall'apprendimento storico (Tier 2). Ottenuta dando alla Expense un `descriptionHash` sintetico (`sha256("detached:{id}")`). La si crea quando l'utente marca una transazione come "tratta come spesa a sé / non aggregare" — tipicamente un **accredito da una persona**, la cui descrizione è una chiave di categorizzazione inaffidabile (lo stesso mittente paga scopi diversi: l'abbonamento condiviso un mese, la cena un altro). L'isolamento è **per-singola-transazione, non una regola permanente**: stacca quella transazione e libera lo slot-hash originale, ma la transazione successiva con la stessa descrizione nasce di nuovo aggregabile. Per una Expense che ha già una sola transazione l'operazione **ri-hasha la riga esistente sul posto** (stesso id, nessun orfano); per una Expense con più transazioni crea una nuova Expense dedicata (servizio `detachTransactionToDedicatedExpense`). Vedi ADR 0016.
_Avoid_: spesa manuale, spesa scollegata, categoria "soldi da persona"

**Expense Group** (Gruppo di spese):
Raggruppamento manuale di N Expense che rappresentano **lo stesso esercente visto da piattaforme diverse** (es. `Cherasco 57` / `CHERASCO 57` / `Cherasco 57 SRL` da tre carte). Ha un titolo proprio e **una sola Subcategory**: il gruppo è l'unità di categorizzazione (ricategorizzare il gruppo propaga a tutti i membri). I membri restano Expense vive e intatte — hash, storico Tier 2 e dedup di re-import invariati; i totali di gruppo sono **calcolati in lettura**, mai persistiti. Nelle liste il gruppo appare come una riga sola (le transazioni mostrano il titolo del gruppo). Verbi UI: **Unisci** (crea gruppo o aggiunge membro a gruppo esistente; richiede stessa Subcategory — le non categorizzate vengono prima categorizzate esplicitamente nel dialog) e **Scomponi** (scioglie il gruppo o rimuove un singolo membro; un gruppo con un solo membro si scioglie da sé). Nessun merge automatico all'import. Vedi ADR 0017.
_Avoid_: merge fisico, spesa fusa, merchant/esercente (non è un'anagrafica), "merged expense" nel codice
...
**Rimborso (refund) — netting per sottocategoria, senza correlazione.** Un accredito che **annulla una spesa specifica** (reso ordine online, rimborso da persona per una spesa condivisa, rimborso sanitario/viaggio di una spesa tracciata) **non è un'entrata**: si categorizza sotto la **stessa sottocategoria della spesa** e **netta per somma algebrica** dentro quel segmento OUT (ADR 0004). Il netting **non richiede** una correlazione transazione↔transazione: una sottocategoria contiene semplicemente transazioni di entrambi i segni e il totale netta per somma algebrica. Esempio: 10 ordini −1000 + 4 resi +300 sotto "shopping online" → −700. _Esiste_ un modello di pairing 1:1 **opzionale** (`transaction_pair`, Phase 50) che lega esplicitamente una spesa e il suo rimborso di segno opposto, usato per chiarezza nelle liste e per il calcolo di `effectiveAmount`: è un di più sopra il netting algebrico, non il meccanismo che lo abilita. Il netting di default resta sempre per sottocategoria, senza alcun link. Distinzione decidibile: _"annulla una mia spesa specifica → netta; è denaro nuovo non legato a una spesa → `income_extraordinary`"_. Conseguenza display: una transazione può avere segno opposto alla direzione della sua nature (un reso `+` sotto una sottocategoria OUT) — in lista si mostra per l'importo reale, nel grafico netta.
...
- **Vacanze** — alloggio, trasporto, assicurazione-viaggio, pacchetto-vacanze (tutte D). _(agg. 2026-07-28: allineato all'audit Phase 67 TAG-06/D-11 — `attivita-e-intrattenimento` e `cibo-e-bevande` sono disattivate. La categoria contiene **solo spesa intrinsecamente di viaggio**; tutto il resto si categorizza per oggetto anche se speso in viaggio. agg. 2026-07-29: `pacchetto-vacanze` è l'oggetto "pacchetto / agenzia / tour operator" — non un remap best-effort su `alloggio` (D-12). Quale viaggio specifico è un asse Tag, non tassonomico.)_
...
- **Cultura e tempo libero** _(merge Tempo libero + Libri e media)_ — spettacoli, libri-e-audiolibri, streaming, app-e-software, videogiochi, attivita-ricreative (tutte D). _(agg. 2026-07-28)_ `spettacoli` _(era `cinema-ed-eventi`)_ è il consumo culturale dal vivo: cinema, teatro, concerti, mostre, musei. `attivita-ricreative` è l'attività ludica occasionale: pattinaggio, bowling, minigolf, escape room, parchi divertimento e tematici. Resta **una sola categoria**: abbonamenti/contenuti digitali (streaming, app, videogiochi) non diventano una categoria a parte — il taglio ricorrenti/abbonamenti è ortogonale (flag/vista), come già stabilito per la nature `operational` sciolta.
  Due discriminanti per questa categoria:
  - **contesto vs oggetto** — il contesto "sono in viaggio" **non** cambia la categoria: `vacanze` accoglie solo la spesa intrinsecamente di viaggio (vedi sopra). Un'ora di pattinaggio in vacanza è `cultura e tempo libero > attivita-ricreative`, e il viaggio si porta con un **Tag**, non con la categoria.
  - **ricreativo vs sportivo** — pratica ricorrente o abbonamento (palestra, corso, piscina) → `benessere > sport-e-fitness`; uscita ludica occasionale → `attivita-ricreative`. La distinzione è la ricorrenza dell'attività, non la disciplina: lo stesso pattinaggio sta in `sport-e-fitness` se è un corso settimanale. Il regex Tier-1 non può vedere la ricorrenza, quindi `pattinaggio` resta mappato su `sport-e-fitness` e i casi occasionali si correggono a mano.
