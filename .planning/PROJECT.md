...
---
*Last updated: 2026-07-27 after v2.8 milestone (Reimbursements 1:N) — shipped all 11 RMB requirements across phases 73–76: 1:N reimbursement model (transaction_pair subsumed), Mondo Netto netting with zero dashboard drift, first-class residual, amount-edit guard generalized to 1:N, linking UI on `/transactions/[id]`, and the dedicated `/reimbursements` list + per-reimbursement page. Group-anchor host descoped (backend dormant). Milestone audit passed 11/11, integration fully wired, full suite green (149 files, 1834 tests). ROADMAP/REQUIREMENTS archived to `milestones/v2.8-*`. Git tag pending post-merge to main (create_tag=false, branching=none)*
