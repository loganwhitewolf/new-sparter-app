...
## Current Milestone: v2.9 Amortization
...
**Target features:**
- Amortization plan lifecycle — amortize an outflow transaction over N months (detach into Standalone Expense, materialised uniform instalments from the purchase month), close a plan (residual collapses onto the closure month), realization via a linked reimbursement.
- Dedicated `/amortizations` registry — every plan with description, date, initial amount, consumed, net value, remaining months, and the close action.
- Global cash/accrual dashboard lens — one switch flips every widget between "cassa" (current) and "competenza" (spread), whole selected year including future instalment months, lens-aware selectors.

**Key context:** Model locked in [ADR 0019](../docs/adr/0019-amortization-accrual-lens.md). Implementation seam is one swappable `ledger_entry` row source per lens (not a `lens` parameter), which structurally prevents the reimbursement double-netting trap. Sits on the v2.8 reimbursement netting; realization reuses it. Charted via the `/wayfinder` map at `.scratch/amortization/`.

## Current State

Milestone v2.8 **shipped 2026-07-27 (audit passed 11/11, git tag pending post-merge)** — all phases 73–76 complete. Phase 73 (reimbursement schema & netting): the 1:1 `transaction_pair` model is generalized into `reimbursement` + `reimbursement_refund` (one outflow anchor → N inflow refunds; XOR CHECK anchor on Expense **or** Expense Group), every existing pair backfilled (migration 0029, sign-based anchor resolution, guarded against silent data loss), `transaction_pair` dropped (0030), the shared `effectiveAmount()`/`isNotSecondary()` netting fragment repointed to the new tables, the D-02 outflow-anchor/inflow-refund invariant enforced at the write path, and dashboard totals proven byte-identical before/after across all 10 aggregation sites via a real-Postgres regression suite. Verified 5/5 must-haves. **Phase 74 (group-anchor-and-reconciliation) complete**: `effectiveAmount()` rewritten as a uniform proportional-spread expression so a refund's net spreads across an anchor's member transactions in proportion to each transaction's own amount (largest-remainder cent assignment) — one mechanism for both Expense and Expense Group anchors, N=1 numerically inert so the Phase 73 regression gate stays green; `residual = Σoutflow + Σrefunds` delivered as a computed, Decimal-safe data-layer value (owed/settled/surplus states, surplus never blocked); the amount-edit guard generalized to protect Group anchors — a code-review gap-closure (74-04) closed CR-01/CR-02 and, via the first real-Postgres test ever run against `updateTransaction()`, also fixed a latent correlated-subquery correlation-ambiguity bug that had silently broken the refund-edit branch for every anchor shape since Phase 73. RMB-02/06/09 validated 9/9 must-haves; full suite green (144 files, 1778 tests). **Phase 75 (linking-surfaces-and-lifecycle) complete**: the outflow-anchored linking surfaces (Expense-detail + Expense-Group linking, a frozen anchored-transaction set, and the unlink/delete lifecycle with pre-link baseline snapshot restore) shipped RMB-07/RMB-08. **Phase 76 (reimbursements-section) complete**: a dedicated `/reimbursements` list (unified-toolbar search/status-filter/sort, colored residual badges, Expense-anchor-only) + a per-reimbursement `/reimbursements/[id]` page (editable title, net/residual/status header, anchor link, add/remove-refund + delete in place, reusing the `/tags/[id]` + `ReimbursementPanel` scaffolding), a transactions row-indicator badge that deep-links to the page, a `/transactions/[id]` read-only summary variant, and a "Rimborsi" sidebar entry; RMB-10/RMB-11 verified, full suite green (149 files, 1833 tests). Two UAT-driven fixes landed at the phase checkpoint: a no-404 redirect to `/reimbursements` when a reimbursement is removed, and removal of the synthetic "rimborso {anchor}" refund-title rewrite (recategorization/netting untouched). **Milestone v2.8 shipped 2026-07-27 — audit passed 11/11, cross-phase integration fully wired, full suite green (149 files, 1834 tests). Requirements/ROADMAP archived to `milestones/v2.8-*`. Git tag pending post-merge to main.**
...
## Last Shipped Milestone: v2.8 — Reimbursements 1:N (shipped 2026-07-27, tag pending post-merge)
...
- [ ] v2.9: Amortization — spread a one-off cost over N months (per-transaction, detach into Standalone Expense), materialised uniform instalments, global cash/accrual dashboard lens (one `ledger_entry` row source per lens), dedicated `/amortizations` registry, realization-as-reimbursement (ADR 0019). In progress.
...
---
*Last updated: 2026-07-29 — milestone v2.9 (Amortization) 100% complete through Phase 81 (inline net display for paired transactions); ready to close. Model locked in ADR 0019: spread a one-off cost over N months per-transaction (detach into Standalone Expense), materialised uniform instalments from the purchase month, global cash/accrual dashboard lens via one swappable `ledger_entry` row source per lens (prevents the reimbursement double-netting trap), dedicated `/amortizations` registry, realization-as-reimbursement reusing v2.8. Charted via the `/wayfinder` map at `.scratch/amortization/`. Previous: v2.8 (Reimbursements 1:N) shipped 2026-07-27, audit 11/11, archived to `milestones/v2.8-*`, git tag pending post-merge.*
