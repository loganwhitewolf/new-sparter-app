...
## Last Shipped Milestone: v2.9 — Amortization (shipped 2026-07-29, tag pending post-merge)
...
## Earlier Shipped Milestone: v2.8 — Reimbursements 1:N (shipped 2026-07-27, tag pending post-merge)
...
- [x] v2.8: Reimbursements 1:N — explicit one-outflow→N-inflows reimbursement (transaction_pair subsumed as N=1), Mondo Netto netting, first-class residual, amount-edit guard generalized, linking UI on `/transactions/[id]`, dedicated `/reimbursements` list + per-reimbursement page (ADR 0018). Group-anchor host descoped (backend dormant). Shipped 2026-07-27 (audit 11/11); git tag pending post-merge.
- [x] v2.9: Amortization — spread a one-off cost over N months (per-transaction, detach into Standalone Expense), materialised uniform instalments, global cash/accrual dashboard lens (one `ledger_entry` row source per lens), dedicated `/amortizations` registry, realization-as-reimbursement (ADR 0019). Phase 81 added inline paired-net display in the transactions table. Shipped 2026-07-29 (audit 15/15); git tag pending post-merge.
- [x] v3.0: Categories Year View — Categories rewritten on a yearly axis: Covered/Partial Month + Ritmo + Proiezione number engine (total = sum of the visible series), list on year + direction with share/sparkline/projection/sort (Accantonamenti reachable), detail as a 12-month table with in-cell deltas, previous-year row, narrowable window and exact-summing subcategory contributions; Deviation/Baseline/Noise-Threshold/Preset retired outright and the cassa/competenza lens confined to Overview (ADR 0020, amends LENS-01 of ADR 0019). Shipped 2026-08-03 (audit 25/25, Nyquist 3/3); git tag pending post-merge. Accepted debt: no browser verification, Accantonamenti not drillable.
