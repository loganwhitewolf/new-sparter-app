...
**Progress:** Phase 82 complete (2026-07-30) — the shared number engine ships as `lib/dal/covered-months.ts` + `lib/services/pace-and-projection.ts`: Covered/Partial Month classification, `Ritmo` as the average of the year's Covered Months, the `max(spent so far, pace)` hybrid current month, `total` built structurally as the sum of the already-rounded series (so PACE-05 holds exactly, not approximately), and one shared per-direction judgement for `current − previous`. The cassa/competenza switch is confined to Overview, with Categories pinned to cassa **by construction** — a branded `LensPassthrough` type forwards `?lens=` through Categories' own URLs so the tab-nav round trip survives (D-13) while making a hand-off to `resolveLedgerRowSource` a compile error (D-12). Dead `tag` param dropped from `buildDashboardTabHref`. RETIRE-05's byte-identical Overview/Tags baseline is captured and now fails loudly rather than skipping when Postgres is unreachable in CI, so Phase 83's `direction.hidden` predicate flip cannot land on a vacuous green. Note the baseline is **preventive**, not detective: Phase 82 added new files rather than modifying the shared aggregation, so nothing could have moved yet. Phases 83 (categories-list) and 84 (category-detail-and-cleanup, incl. RETIRE-01/02) remain.

**Known gap:** the Playwright suite (`tests/dashboard.spec.ts`) is not executable in the current dev environment — every test in its LENS block fails with `net::ERR_TOO_MANY_REDIRECTS`, including ones untouched by v3.0, an auth/proxy redirect loop upstream of this work. Phase 82's two e2e edits are source-verified and typechecked but **unexecuted**; the behaviour they assert is covered by `tests/lens-switch-placement.test.tsx` in the green vitest suite. Also tracked: 13 other real-Postgres suites still skip silently without Docker (`amortization-lens-regression*.test.ts` are the obvious next adopters of the new CI guard).
