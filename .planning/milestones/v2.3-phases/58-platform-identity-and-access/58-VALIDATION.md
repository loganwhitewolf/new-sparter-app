...
| 58-02-01 | — | — | PLAT-02 | — | A `pending` platform is visible only to its `proposedByUserId`; an `approved` platform (incl. all seeded) is visible to every user | unit | `yarn test tests/import-private-formats-dal.test.ts` (PLAT-02 cases folded into this file) | ❌ W0 | ⬜ pending |
