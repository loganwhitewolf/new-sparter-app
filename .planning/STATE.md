...
Last activity: 2026-08-07 - Completed quick task 260807-l2c: the ?nature= filter allowlist is now derived from the FlowNature union (savings/investment were silently dropped on /transactions and /expenses) — step 1 of .planning/SEEDS-ROADMAP.md
...
| 260807-l2c | Fix filtro Natura: `?nature=savings` e `?nature=investment` venivano **scartati in silenzio** su `/transactions` e `/expenses` — entrambi i validatori mantenevano a mano un `NATURE_ALLOWED` con i 3 codici morti di v2.0 (`operational`/`financial`/`extraordinary`) e **senza** i 2 codici vivi, mentre le opzioni del filtro arrivano da `NATURE_ORDER` (8 codici corretti): l'utente selezionava Risparmio/Investimento e la tabella restava non filtrata. Nuovo `NATURE_FILTER_VALUES` in `lib/utils/nature-labels.ts` derivato da `FLOW_NATURE_MEMBERS: Record<FlowNature, true>` (esaustività a compile-time verificata: una nona nature fa fallire `tsc` con TS2741), le due copie locali cancellate. `lib/validations/category.ts` e `TYPE_ALLOWED` intatti; nessun alias legacy (i 3 codici morti erano già scartati); nessuna lettura da DB (variante bocciata in SEED-003). Nuovo `tests/nature-filter-allowlist.test.ts` con matrice 9 accettati × 2 parser + 4 rifiutati × 2 parser, lista hardcoded per non auto-assolvere la derivazione. Passo 1 di `.planning/SEEDS-ROADMAP.md` | 2026-08-07 | 8390805d, 7420b5f9 |
