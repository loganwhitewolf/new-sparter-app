...
provides:
  ...
  - "PO verdict recorded: Winner = c (Type-led stack), with Steal-from-losers and Do-not-ship notes for Phase 75"
...
key-decisions:
  ...
  - "Do not ship a or b as the primary homepage layout; no components/marketing/* extraction in this phase (D-09) — that is Phase 75's job"
...
coverage:
  ...
  - id: D2
    description: "After human Preview review, Verdetto PO records the winning variant (c) plus handoff notes for Phase 75; REQUIREMENTS.md BRAND-02 checkbox follows the written verdict, not the other way around"
    requirement: "BRAND-02"
    verification:
      - kind: other
...
# Phase 73 Plan 03: NOTES.md Design-Lock Template + PO Verdict Summary

**BRAND-02 closed: `app/proto/branding/NOTES.md` design-lock ritual shipped and filled — PO picked Variant C (Type-led stack) as the production direction, with explicit steal/do-not-ship notes for Phase 75.**
...
- PO verdict recorded after Preview compare: **Winner = c (Type-led stack)**; Steal from losers = a's dominant product-visual weight + b's editorial type/asymmetric-column treatment; Do not ship = a/b as primary homepage layout, and no `components/marketing/*` extraction yet (D-09, Phase 75's job)
...
- `app/proto/branding/NOTES.md` - Design-lock handoff for Phase 75: Domanda/Come provarlo/Varianti template (Task 1), then filled Verdetto PO with Winner=c, Steal from losers, Do not ship (Task 2)
...
- **Steal-from-losers notes are directional, not a task list** — they flag which visual qualities of a (dominant product screenshot) and b (editorial type weight, asymmetric column) are worth carrying into the Phase 75 production build of C, without prescribing exact implementation. That's Phase 75's planning job.
...
- **BRAND-02 is satisfied.** `app/proto/branding/NOTES.md` is the locked design-lock artifact: Winner = c (Type-led stack), with explicit steal/do-not-ship notes. Phase 75 (marketing-pages) can read this file directly as its design-direction input — no further PO sign-off needed for the *which layout* question.
- Phase 75 should treat Variant C's structure (`app/proto/branding/variant-c.tsx`) as the closest analog for the production homepage, while pulling in the noted steal items (a's product-visual weight, b's editorial type treatment) during that phase's own planning — this plan intentionally does not prescribe how.
- Per D-09, no `components/marketing/*` extraction happened in this phase — `app/proto/branding/**` remains throwaway, Preview-gated, `noindex`. Phase 75 owns the promotion.
- No blockers. Phase 73 (proto-design-variants) is now fully complete: BRAND-01 (Plan 02) and BRAND-02 (this plan) are both satisfied.
