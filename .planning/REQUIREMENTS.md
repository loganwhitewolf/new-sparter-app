...
- [ ] **DET-01** — `updateTransaction` service+action: edit `amount` (Decimal.js,
  signed), `occurredAt`, `customTitle`, all inside `db.transaction`; hashes and
  `description` untouched. Zod validation; ownership-gated.
- [ ] **DET-02** — Reconciliation: after an amount/date edit, the linked expense's
...
- [ ] **DET-03** — Pair guard: editing a paired transaction's amount is rejected
