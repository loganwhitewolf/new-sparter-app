...
Sparter migrates from Express + Sequelize to Next.js 15 App Router + Drizzle ORM in 7 sequential phases. The journey starts with the design system and authentication shell, builds toward manual expense management and dashboard KPIs, then delivers the full CSV import pipeline with advanced regex categorization, and closes with the user profile screen. Every phase leaves the app in a coherent, usable state.

## Stack Context

All plan-phase agents must be aware of these constraints:

- **Stack**: Next.js 15 App Router, Drizzle ORM + MySQL, NextAuth v5 (Credentials, JWT), Cloudflare R2, Zod, Decimal.js, Tailwind CSS + shadcn/ui
- **DAL pattern**: `lib/dal/` (Drizzle queries) → `lib/services/` (business logic) → `lib/actions/` (server actions, thin wrappers)
- **Seed data**: `docs/init/seed.ts` contains 26 categories, ~120 subcategories, 6 platforms, 28 regex patterns — port to `drizzle/seed.ts`
...
- **Import preview**: Required in Phase 5 — show row count, duplicate count, sample rows before committing the import
- **NextAuth v5**: Config at root `auth.ts`; use `auth()` not `getServerSession()`; return `null` (not `false`) from `authorize`; custom fields (`subscriptionPlan`, `role`, `userId`) must be in `jwt` + `session` callbacks and TypeScript augmentation
- **R2 upload**: Presigned PUT URL generated by Route Handler → browser uploads directly to R2 → second call confirms. Never proxy file upload through server actions
...
- [ ] **Phase 5: File Import** - Upload CSV/Excel su R2, analisi colonne, preview, import con deduplicazione
- [ ] **Phase 6: Import Avanzato & Categorizzazione** - Pattern regex custom utente, history-based Tier 2, subscription gating
...
### Phase 5: File Import
**Goal**: L'utente può importare un file CSV o Excel della propria banca — il file viene caricato su R2, il sistema riconosce la piattaforma, mostra un'anteprima e importa le transazioni con deduplicazione
**Depends on**: Phase 2, Phase 3
**Requirements**: IMP-01, IMP-02, IMP-03, IMP-04
**Success Criteria** (what must be TRUE):
  1. L'utente può caricare un file CSV o Excel e vederlo apparire nella lista con status `pending`; il file è su Cloudflare R2
  2. Il sistema propone automaticamente la piattaforma bancaria più compatibile; l'utente può confermare o scegliere manualmente
  3. Prima di confermare l'import, l'utente vede: numero di righe rilevate, numero di duplicati già importati, sample di righe parsed
  4. Dopo la conferma, le transazioni vengono importate senza duplicati (transactionHash), aggregate in expense per descrizione normalizzata (descriptionHash) e sottoposte alla pipeline di categorizzazione Tier 1 + Tier 2 secondo il piano di subscription
**Plans**: TBD
**UI hint**: yes
