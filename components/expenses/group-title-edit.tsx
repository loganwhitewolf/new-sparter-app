...
import { renameExpenseGroupAction } from '@/lib/actions/expenses'
...
export function GroupTitleEdit({ groupId, title, onSuccess }: Props) {
  const [isEditing, setIsEditing] = useState(false)
  const [value, setValue] = useState(title)
  const [state, formAction, isPending] = useActionState(renameExpenseGroupAction, {
    error: null,
  })
  ...
  if (!isEditing) {
    return (
      <div className="flex min-w-0 max-w-full items-center gap-1">
        <span className="block min-w-0 flex-1 truncate font-mono text-sm tracking-tight" title={title}>
          {title}
        </span>
        <button
          type="button"
          ...
          title="Clicca per modificare il nome di questo gruppo"
        >
    ...
    <div className="flex min-w-0 flex-col gap-1">
      ...
      >
        ...
        <p className="text-xs text-muted-foreground">
          Modifica il nome del gruppo, senza cambiare le spese collegate.
        </p>
        {state.error ? (
