...
async function loadImportFile(database: DbOrTx, input: { userId: string; fileId: string }) {
...
function validateFileId(input: { userId: string; fileId: string; phase: 'preview' | 'delete.load' }) {
...
async function loadLinkedTransactions(database: DbOrTx, input: { userId: string; fileId: string }) {
  return database
    .select({ id: transactionTable.id, expenseId: transactionTable.expenseId })
    .from(transactionTable)
    .where(and(eq(transactionTable.userId, input.userId), eq(transactionTable.fileId, input.fileId))) as Promise<LinkedTransactionRow[]>
}
...
async function loadRemainingAggregates(
  ...
  return database
    ...
    .groupBy(transactionTable.expenseId) as Promise<RemainingExpenseAggregate[]>
}
...
async function buildImpactPlan(database: DbOrTx, input: { userId: string; fileId: string; phase: 'preview' | 'delete.load' }) {
  ...
  const affectedExpenseIds = [...new Set(linkedTransactions.map((row) => row.expenseId).filter(Boolean) as string[])]
  const remainingAggregates = await loadRemainingAggregates(database, { ...input, affectedExpenseIds })
  const manuallyPreservedExpenseIds = await loadManualOrOverrideExpenseIds(database, {
    userId: input.userId,
    affectedExpenseIds,
  })
  ...
  const remainingByExpenseId = new Map(
    remainingAggregates
      .filter((row): row is RemainingExpenseAggregate & { expenseId: string } => Boolean(row.expenseId))
...
export async function getImportDeletePreview(input: { userId: string; fileId: string }): Promise<ImportDeletePreview> {
  try {
    ...
    throw new ImportDeleteError('preview_failed', 'Impossibile calcolare l’impatto dell’eliminazione.')
  }
}

async function updateRecalculatedExpenses(database: DbOrTx, plan: ImpactPlan, userId: string) {
...
async function preserveEmptyManualExpenses(database: DbOrTx, plan: ImpactPlan, userId: string) {
...
async function deleteEmptyExpenses(database: DbOrTx, plan: ImpactPlan, userId: string) {
...
export async function deleteImport(input: { userId: string; fileId: string }): Promise<ImportDeleteResult> {
  validateFileId({ ...input, phase: 'delete.load' })

  try {
    const result = await db.transaction(async (tx) => {
      const plan = await buildImpactPlan(tx, { ...input, phase: 'delete.load' })

      await tx
        .delete(transactionTable)
        .where(and(eq(transactionTable.userId, input.userId), eq(transactionTable.fileId, input.fileId)))
    ...
    throw new ImportDeleteError('delete_failed', 'Impossibile eliminare l’importazione.')
  }
}
